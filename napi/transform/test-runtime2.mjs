...
const t1 = `var old; const dec = () => (cls, ctx) => { old = cls; }; var Foo = @dec() class {}; assertEq(() => Foo, old);`;
...
try {
  const fn = new Function(`"use strict"; function assertEq(cb, expected) { var x = cb(); if (x === expected) return true; console.log('FAIL: x=' + x + ' expected=' + expected + ' typeof x=' + typeof x + ' typeof expected=' + typeof expected); return false; } ${result.code}`);
  fn();
